...
export default function AiChatView({ currentUser }: AiChatViewProps) {
  ...
  if (!nexusAiUser) {
    allUsersAndBots.push({ id: 'nexus-ai', name: 'Nexus AI', avatar: 'https://picsum.photos/seed/999/40/40', status: 'online' });
  }
  ...
  const sendMessage = async (content: string) => {
    ...
    setTimeout(() => {
      const botMessage: Message = {
        id: `msg - bot - ${Date.now()} `,
        senderId: "nexus-ai",
        content: `I'm currently in prototype mode. You asked: "${content}". Once the backend is connected, I can provide real assistance!`,
        timestamp: format(new Date(), "p"),
        channelId: "ai-chat",
        type: "bot",
      };
        ...
        <div className="p-4 space-y-4">
          {messages.length === 0 && (
            <div className="text-center text-muted-foreground mt-8">
              ...
              <h3 className="text-lg font-semibold">Welcome to Nexus AI (Prototype)</h3>
